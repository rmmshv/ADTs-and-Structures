...
bool LinkedList<T>::deleteNode(const T &target)
{
  ...
  // Find node containing the target, skip other nodes
  while (pCur && pCur->getData() < target) {
      pCur = pCur->getNext();
  }
  // If found, delete the node
  if (pCur && pCur->getData() == target) {
...
void LinkedList<T>::displayListBack() const
{
   ...
   // Skip the head and point to the tail of the list
   pCur = head->getPrev();

   while (pCur != head) {
       // Displaye the value in this node
       std::cout <<pCur->getData();
       // Move to the next node
       pCur = pCur->getPrev();
   }
...
bool LinkedList<T>::searchList(const T &target, T &dataOut) const
{
   ...
   ListNode<T> *pCur; // To move through the list
    ...
    // Position pCur, skip the head of the list
   pCur = head->getNext();
   // skip all nodes less than target
   while (pCur != head && pCur->getData() < target) {
       pCur = pCur->getNext();
   }
   // If found, copy data to the output patameter and change the flag tot true
