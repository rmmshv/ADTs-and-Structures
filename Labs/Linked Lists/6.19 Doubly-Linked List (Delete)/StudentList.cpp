...
{
    ListNode *pCur; // To move through the lsit

    // Position pCur: skip the head and point to the last node
    pCur = head->back;
    ...
    while (pCur != head) {
        // Display the value in this node
        cout <<pCur->stu.gpa << " " <<pCur->stu.name <<endl;
        // Move to the next(prescending) node
        pCur = pCur->back;
    }
    cout <<endl;
}
...
bool StudentList::deleteNode(string target)
{
    ...
    // While pCur points to a node, traverse the list
    while (pCur != head) {
        // if node is not the target go to the next node
        if (pCur->stu.name != target) {
            pCur = pCur->forw;
        } else { // if found, delete the node
